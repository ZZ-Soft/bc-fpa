# Documentazione tecnica — FatturaPA per Business Central online

Riferimento completo dell'estensione: modello dati, flusso SdI, export delle fatture di
vendita, vincoli architetturali, patch ai fogli di stile, struttura del codice, setup e
limiti noti.

Per la presentazione del progetto e i primi passi: [`README.md`](../README.md).
...
```
FPA Xml File                    (73000)   la "cartella": 1 record per file fisico
  ...
  └─ se RICEVUTA:              │
      ...
      Error Count, Receipt Note
        ...
        │ File Name
        ▼
FPA Xml File Document           (73001)   1 record per FatturaElettronicaBody
...
uno bastano un `value` in `enum "FPA Receipt Type"` e una riga in `ReceiptTypeFromCode`.
...
correggere. Da qui la tabella `FPA Receipt Error` (73003), una riga per errore, con sottopagina
...
`FPA Xml File`. Anche l'XML resta uno solo, sul padre.
...
`FPA Body Extractor.ExtractBody` costruisce al volo un XML ridotto — header + **solo quel body** —
...
di AL: codeunit `FPA Xml File Manager` (73008).
...
```
Posted Sales Invoice ──► FPA Xml File Manager ──► TempBlob ──► FPA Xml File
   azione "Generate and File XML"    (XmlDocument)            + esplosione documenti
```

`FPA Sales Export` (73007) continua a decidere soltanto **come si chiama il file e dove finisce**.

> Residuo da ripulire: `FPA Sales Export Setup` ha ancora il campo `Electronic Format`, con
...
`FPA VAT Summary Buffer` (73005) e scritte dopo. La tabella è `TableType = Temporary`: non tocca
mai il database, esiste solo per dare al raggruppamento una chiave che il runtime mantiene.

### Anteprima senza conseguenze

`FPA Xml Test` (73009) costruisce l'XML di un documento registrato e lo offre in download
**senza scrivere niente**: nessun record in `FPA Xml File`, nessuna esplosione in documenti e
...
AppSourceCop.json                     mandatoryAffixes: FPA
LICENSE  NOTICE  CONTRIBUTING.md      Apache-2.0 + attribuzioni di terze parti

src/
  --- tabelle -------------------------------------------------------------
  FPAXmlFile.Table.al                   73000  la cartella: 1 record per file
  FPAXmlFileDocument.Table.al           73001  1 record per FatturaElettronicaBody
  FPAXsdSchema.Table.al                 73002  XSD caricati
  FPAReceiptError.Table.al              73003  1 riga per <Errore> di uno scarto
  FPASalesExportSetup.Table.al          73004  setup export vendite
  FPAVatSummaryBuffer.Table.al          73005  DatiRiepilogo (TableType = Temporary)
  FPASdiCue.Table.al                    73006  contatori del role center (FlowField)

  --- enum ----------------------------------------------------------------
  FPAValidationStatus.Enum.al           73000
  FPAFileType.Enum.al                   73001  Invoice / Receipt / Unknown
  FPAReceiptType.Enum.al                73002  RC / NS / MC / MT / Other
  FPASdiStatus.Enum.al                  73003  esito della fattura a SdI, 10 valori
  FPAFileOrigin.Enum.al                 73004  Upload / Sales Export
  FPAProgressivoSource.Enum.al          73005  Sequential / Random / From Document
  FPASourceDocType.Enum.al              73006  Sales Invoice / Sales Credit Memo

  --- codeunit ------------------------------------------------------------
  FPAXmlReader.Codeunit.al              73000  import multiplo + classificazione + esplosione
  FPAXsdValidator.Codeunit.al           73001  validazione via Codeunit "Xml Validation"
  FPABodyExtractor.Codeunit.al          73002  XML ridotto a un singolo body
  FPAChunkHelper.Codeunit.al            73003  spezzettamento per il control add-in
  FPAFileNameParser.Codeunit.al         73004  nome file -> tipo, base name, codice ricevuta
  FPASdiStatusMgt.Codeunit.al           73005  ricevute -> esito sulla fattura
  FPAProgressivoMgt.Codeunit.al         73006  nome file SdI per le vendite
  FPASalesExport.Codeunit.al            73007  archiviazione dell'uscita in FPA Xml File
  FPAXmlFileManager.Codeunit.al         73008  COSTRUZIONE dell'XML FatturaPA (XmlDocument)
  FPAXmlTest.Codeunit.al                73009  dry run: XML in download, niente scritture
  FPAAppUpgrade.Codeunit.al             73048  Upgrade: esegue 73099
  FPAAppInstaller.Codeunit.al           73049  Install (trigger oggi vuoti)
  FPASalesHeaderSubscriber.Codeunit.al  73088  blocca la registrazione senza
                                               Payment Method / Payment Terms
  FPABase.Codeunit.al                   73099  popola i codici MP** e le nature IVA

  --- pagine --------------------------------------------------------------
  FPAXmlFiles.Page.al                   73000  lista file  (fileuploadaction)
  FPAXmlFileCard.Page.al                73001  card file + sottopagina documenti
  FPAXmlFileViewer.Page.al              73002  anteprima del file intero
  FPAXmlFileDocuments.Page.al           73003  lista documenti (fileuploadaction)
  FPAXmlFileDocSubform.Page.al          73004  sottopagina documenti
  FPAXmlFileDocViewer.Page.al           73005  anteprima di UN documento
  FPAXsdSchemas.Page.al                 73006  XSD (fileuploadaction)
  FPAReceiptSubform.Page.al             73007  ricevute di una fattura
  FPAReceiptErrors.Page.al              73008  errori di uno scarto
  FPASalesExportSetup.Page.al           73009  setup export vendite
  FPASdiRoleCenter.Page.al              73020  role center FatturaPA / SdI
  FPASdiActivities.Page.al              73021  riquadro dei tile del role center

  --- estensioni di pagina ------------------------------------------------
  FPAPostedSalesInvoice.PageExt.al      73010
  FPAPostedSalesInvoices.PageExt.al     73011  export multiplo dalla lista
  FPAPostedSalesCrMemo.PageExt.al       73012
  FPAPostedSalesInvUpdate.PageExt.al    73097  Payment Method Code su
                                               "Posted Sales Invoice - Update"

  --- altro ---------------------------------------------------------------
  FPAViewer.PermissionSet.al            73000
  FPASdi.Profile.al                            rende il role center selezionabile
  FPAStylesheetViewer.ControlAddIn.al
...
Translations/
  FatturaPA.g.xlf                      generato dal compilatore (TranslationFile)
...
Prefisso oggetti, suffissi dei file e namespace sono uniformi su tutti e 47 gli oggetti:

- **affisso `FPA`** su ogni oggetto, come pretende `AppSourceCop.json` (`mandatoryAffixes`),
  compresi quelli aggiunti dopo il primo impianto — `FPA Base`, `FPA App Installer`,
  `FPA App Upgrade`, `FPA Sales Header Subscriber`, `FPA Posted Sales Inv. Update` — e sui
  controlli e le azioni innestati nelle pagine standard, dove AS0011 colpisce il singolo
  controllo e non solo l'oggetto;
- **nomi file** `<Oggetto>.<Tipo>.al`;
- **`namespace ZZSoft.FPA;`** in testa a ogni file.

I tre file entrati per ultimi nel namespace — `FPABase.Codeunit.al`,
`FPASalesHeaderSubscriber.Codeunit.al` e `FPAPostedSalesInvUpdate.PageExt.al` — stavano nel
namespace globale, da cui si vede tutto senza dichiarare nulla. Portarli dentro `ZZSoft.FPA`
ha reso necessari i `using` che prima erano impliciti: `Microsoft.EServices.EDocument` e
`Microsoft.Finance.VAT.TransactionNature` per `FPA Base` (usa `Fattura Code` e
`VAT Transaction Nature`), `Microsoft.Sales.Document` per il subscriber su `Sales Header`,
`Microsoft.Sales.History` per l'estensione di `Posted Sales Invoice - Update` (la pagina
12211 della localizzazione italiana, non la 1355 standard).

Due identificatori conservano di proposito il vecchio prefisso e **non** vanno rinominati:

- `Label 'FE-PROGRESSIVO'` in `FPAProgressivoMgt.Codeunit.al` è il nome della
  `NumberSequence`. È un dato persistito: rinominarlo creerebbe una sequenza nuova che
  riparte da zero, e il progressivo già consumato andrebbe perso.
- gli asset del viewer (`fe-sef.js`, `fe-viewer.js`, `fe-startup.js`, `fe-viewer.css` e la
  variabile globale `window.FE_SEF`) non sono oggetti AL: AppSourceCop non li vede, e
  rinominarli vorrebbe dire toccare i cinque path del `controladdin` e `tools/build-sef.sh`
...
che contiene il file .al**. `FPAStylesheetViewer.ControlAddIn.al` sta in `src/`, quindi gli asset
...
- **Role center FatturaPA / SdI** → reso selezionabile da *My Settings* dal profilo `FPA SDI`.
  I tile contano su `FPA SDI Cue`, i cui campi sono tutti FlowField su `FPA Xml File`: il numero
...
`FPAXmlReader` portano il prefisso `a:` solo sulla radice, e la navigazione dentro il body usa
...
- **Validazione XSD delle ricevute**: il validatore usa gli schemi caricati in tabella, che sono
  ...
  `targetNamespace`.
- **`Electronic Format` è un campo morto**: `FPA Sales Export Setup` lo espone ancora, con
  ...
  `FPA Xml File Manager`, quel campo non determina più nulla: va rimosso insieme ai suoi tooltip.
...
- **Dimensione**: l'XML viaggia al client a blocchi di 60.000 caratteri (`FPA Chunk Helper`).
